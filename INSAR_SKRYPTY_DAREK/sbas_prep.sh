...
printf '\tglobal data\n' >> imglist.py
printf '\tdata = imp.load_source('\''data'\'', filename, f)\n' >> imglist.py
printf '\tf.close()\n\n' >> imglist.py
...
cp ../F2/baseline_table.dat .       #if you have baseline_table.dat, unhash the next three lines

awk '{print $1, $3}' baseline_table.dat | sed 's/_ALL_F2//g' | sed 's/S1_//g' > scene.tab 

paste -d '\t' intf.tab imglist.tmp > intftab.tmp
rm intf.tab

awk '{print "V"$1" = "$5}' baseline_table.dat > bperp.tmp
...
echo "command: sbas intf.tab scene.tab "$N $S $xdim $ydim "-range 888126 -incidence 40 -wavelength 0.0554658"
